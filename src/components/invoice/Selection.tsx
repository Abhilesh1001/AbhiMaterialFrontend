    ...
    <div className='overflow-auto' style={{ display: 'flex' }}>
...

        {
            ...
            <PrBurron label={'Change'} onClick={handleGrnchange} />
                {irnchange && <>{hastruevalue?<button className="btn btn-success mx-2  text-gray-800 dark:bg-green-500 bg-green-500 dark:text-gray-50 h-8 text-sm" type='button' onClick={()=>handleUpdateGRN(Number(irndata.mir_no))} >Update</button>:<PrBurron label='Update'/>}</>}
            </>
        }
        {
            selectedValue === 'PO' && <> 
